...
1. **Frontmatter shape** — only these keys, in any order:
   ...
   description: <triggers>     # 1–1024 chars, must include "Bevy 0.19"
   license: MIT
...
3. **Version-pinned, always.** Every skill body opens with a "Bevy 0.19" heading. Every API is version-honest. If an API changed in 0.18, say so in the Gotchas section.
...
1. **Verify APIs.** Run `cargo doc --open` against `bevy = "0.19"` in the `bevy-skills-tester` crate (a separate sibling repo, see below) or browse https://docs.rs/bevy/0.19 . Do not trust your training data — Bevy moves fast.
...
4. Description contains "Bevy 0.19" verbatim and names ≥3 concrete triggers.
