...
Rules enforced (see CLAUDE.md "Hard rules"):
  ...
  - `description` is 1-1024 chars and contains its pinned target version
    ("Bevy 0.19" by default; metadata.target_version may override historical skills).
...
def lint_file(path: Path) -> LintResult:
    ...
    if isinstance(desc, str):
        ...
        if not isinstance(expected_pin, str) or not expected_pin:
            expected_pin = REQUIRED_VERSION_TOKEN  # "Bevy 0.19"
