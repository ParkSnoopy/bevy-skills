...
**Read this first when working on a Bevy project.** All sibling skills assume Bevy 0.19 (released 2026-06-18). If the user's `Cargo.toml` pins a different version, stop and confirm before applying patterns from this collection.
...
| Writing an `AssetLoader` (must `#[derive(TypePath)]`) | `bevy-custom-assets` |
...
| `StandardMaterial`, `MaterialPlugin<M>`, `AsBindGroup::label()`, PBR shading | `bevy-pbr-materials` |
...
1. **Events are messages.** `EventReader<E>` and `EventWriter<E>` were renamed to `MessageReader<M>` / `MessageWriter<M>` in 0.17. Still wrong if Claude writes the old name in 0.19.
...
6. **`RenderTarget` is its own component** in 0.19, not a field on `Camera`.
7. **`Mesh::insert_attribute` → `try_insert_attribute(...)?`** — returns `Result` in 0.19.

## Gotchas

- Bevy's training-data footprint is dominated by 0.10–0.15 code. If a snippet feels obvious, double-check it against both migration skills before trusting it.
