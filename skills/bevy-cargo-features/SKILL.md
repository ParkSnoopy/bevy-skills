...
`2d`, `3d`, `ui` are the three top-level **feature collections** carried in 0.19. They turn on a curated stack appropriate for that app shape. Use these instead of hand-picking 40 individual flags.
...
`2d_api`, `3d_api`, `ui_api` enable the **API surface** (component types, asset types, shader types) without enabling the high-level renderer plugins. Available in 0.19.
...
In 0.19, `default-features = false` no longer turns on input by default. Opt in to `mouse`, `keyboard`, `gamepad`, `touch`, `gestures` individually.
...
- **`multi_threaded`** is a separate Bevy feature in 0.19. On by default through the high-level collections; off in headless minimal builds unless you ask for it.
