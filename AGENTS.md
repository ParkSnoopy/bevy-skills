...
A collection of Agent Skills (`SKILL.md` files) that teach AI agents how to write correct **Bevy 0.19** code, with explicit Bevy 0.18 exceptions. Targets Claude Code, OpenCode, Cursor, Codex, Pi, and any other Agent Skills consumer.
...
- Every code snippet must `cargo check` against the skill's `metadata.bevy_version` in the companion `bevy-skills-tester` crate (not in this repo — see `CLAUDE.md`).
- Pin the targeted Bevy version explicitly in every skill's description. No unversioned guidance.
