...
A collection of Agent Skills (`SKILL.md` files) that teach AI agents how to write correct **Bevy 0.19** code. Targets Claude Code, OpenCode, Cursor, Codex, Pi, and any other Agent Skills consumer.
...
- Every code snippet must `cargo check` against `bevy = "0.19"` in the companion `bevy-skills-tester` crate (not in this repo — see `CLAUDE.md`).
- Pin Bevy 0.19 explicitly in every current skill's description. Historical
  migration skills pin their own target with `metadata.target_version`; no
  guidance is unversioned.
- Use `dex` for persistent project work. Repository-local tasks live in `.dex/tasks.jsonl` and automatically sync root tasks to GitHub Issues.
